...
.sainet-header{background:linear-gradient(135deg,#040E1C 0%,#112A47 100%);border-bottom:3px solid #F0A500;padding:1.1rem 2.2rem;display:flex;align-items:center;gap:1.4rem;width:100%}
...
with tab1:
    ...
    if run and name.strip():
        ...
        if cmpd is None:
            st.error('**{}** not found on PubChem. Try the generic name (e.g. donepezil not Aricept).'.format(name))
            st.stop()
        st.markdown('''
        ...
        if err or not models:
            st.error('Model loading failed: {}'.format(err or 'no checkpoints'))
            st.stop()
        with st.spinner('Running SAI-Net v3 ensemble prediction...'):
            result = predict(cmpd['smiles'], models)
        if result is None:
            st.error('Prediction failed. Compound structure may not be compatible.')
            st.stop()
        ...
        with cs:
            ...
            st.markdown('<div class="pred-label" style="margin-top:1rem">CNS Rules</div>', unsafe_allow_html=True)
            for prop, (val, ok) in cns_rules(cmpd['smiles']).items():
                c, bg = ('#1B6B45','#E6F5EE') if ok else ('#9B2335','#FDE8EA')
                icon = '&#10003;' if ok else '&#10007;'
                st.markdown('<div style="display:flex;justify-content:space-between;padding:.4rem .7rem;background:{bg};border-radius:6px;margin-bottom:.3rem"><span style="font-size:.82rem;font-weight:600;color:{c}">{prop}</span><span style="font-size:.82rem;font-weight:700;color:{c}">{icon} {val}</span></div>'.format(bg=bg,c=c,prop=prop,icon=icon,val=val), unsafe_allow_html=True)
        with cc:
        ...
        with cr:
            ...
            if bbb >= 0.5 and lbb >= 0.30 and pgp < 0.5:
                ...
                vc, vt, vb = 'e', 'EFFLUX LIMITED', 'PGP {:.0%}'.format(pgp)
            else:
                vc, vt, vb = 'r', 'CNS-RESTRICTED', 'BBB {:.0%} logBB {:+.2f}'.format(bbb, lbb)
            st.markdown('<div class="verdict-{vc}"><div style="font-size:.70rem;font-weight:800;text-transform:uppercase;letter-spacing:1.4px;margin-bottom:.3rem">CNS Profile Verdict</div><div style="font-size:1.05rem;font-weight:700">{vt}</div><div style="font-size:.82rem;margin-top:.2rem">{vb}</div></div>'.format(vc=vc,vt=vt,vb=vb), unsafe_allow_html=True)
        st.download_button('Download Report (JSON)',
            data=json.dumps({'compound': name, 'cid': cmpd['cid'],
        ...
        st.markdown('<div class="disclaimer"><b>Educational Purpose Only</b> - Not intended as medical advice.</div>', unsafe_allow_html=True)
    elif run:
...
with tab2:
    ...
    with cl:
        if SAI_SRC:
            st.markdown('<div style="padding:1rem"><img src="{}" style="width:160px;height:160px;border-radius:50%;border:3px solid #F0A500"></div>'.format(SAI_SRC), unsafe_allow_html=True)
    with ct:
        st.markdown('''<div class="about-card">
          <div style="font-size:1.6rem;font-weight:800;color:#0D2137">About <span style="color:#F0A500">SAI BBB</span> Predictor</div>
          <div style="background:#F0A500;height:3px;width:60px;border-radius:2px;margin:.5rem 0 1rem"></div>
